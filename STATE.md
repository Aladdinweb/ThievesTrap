...
**Current version:** `v2.8.6` (versionCode 130) -- reverted to this exact baseline 2026-08-28 at the user's explicit request, after Face Capture (v2.8.7) work caused instability. See Section 8 for full revert history and Section 7 for a critical gotcha about this tag.
...
- **Unresolved bugs found and diagnosed during the v2.8.7 Face Capture attempt (2026-08-28), NOT currently fixed in this v2.8.6 baseline** -- documented here so this investigation isn't lost, even though the code changes were reverted at the user's request to prioritize stability. If SMS commands or wrong-PIN photo capture seem unreliable in a future session, start here instead of re-diagnosing from scratch:
  ...
  - None of the above four fixes are currently applied in this v2.8.6 baseline. They were verified individually (each one built green and was reasoned through with actual code evidence, not guessed) before the user asked for a full revert due to overall instability. A future attempt at any of these should re-apply them one at a time with a real device retest between each, rather than all at once.
