...
class SurvivalTimerService : Service() {
    ...
    companion object {
        ...
        const val WARN_5MIN    = 5 * 60 * 1000L   // standard warning
        const val WARN_1MIN    = 1 * 60 * 1000L   // short-timer warning
        const val SMS2_DELAY   = 15 * 60 * 1000L  // 15 min second SMS
    ...
    private var warnBefore   = WARN_5MIN  // dynamic: 5min or 1min
        ...
        override fun run() {
            ...
            when {
                remaining <= 0 && !sms1Sent -> {
                    ...
                    handler.postDelayed({ if (!sms2Sent) { sms2Sent = true; sendSurvivalSMS(2) }
                        autoStop() }, SMS2_DELAY)
                }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // Fix 4: if timer <= 5 min, warn at 1 min instead of 5 min
    ...
    private fun vibrate5Min() = try {
        // Warning vibration — only at 5-min mark, NOT at start
        val v = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)
            (getSystemService(VibratorManager::class.java)).defaultVibrator
        else @Suppress("DEPRECATION") getSystemService(VIBRATOR_SERVICE) as android.os.Vibrator
        // 3 long pulses to alert user — then stop (repeat = -1 means no loop)
    ...
    private fun buildNotif(text: String): Notification {
        val pi = android.app.PendingIntent.getBroadcast(
            this, 0,
            Intent(ACTION_IM_SAFE).apply {
                setPackage(packageName)
                setClass(this@SurvivalTimerService, SafeConfirmReceiver::class.java)
            },
    ...
    private fun createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val ch = NotificationChannel(CHAN_ID, "Survival Timer",
                ...
                setSound(null, null)   // no sound during countdown
                enableVibration(false) // no vibration during countdown
            }
