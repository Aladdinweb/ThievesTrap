...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun loadAll() {
        ...
        // ── EMAIL ALERTS section (v2.8.8) ──
        try {
            val swEmail   = findViewById<Switch>(R.id.sw_email_alerts)
            val etFrom    = findViewById<EditText>(R.id.et_email_from)
            val etPass    = findViewById<EditText>(R.id.et_email_password)
            val etTo      = findViewById<EditText>(R.id.et_email_to)
            val btnTest   = findViewById<Button>(R.id.btn_email_test)
            val btnSave   = findViewById<Button>(R.id.btn_email_save)
            val btnInfo   = try { findViewById<Button>(R.id.btn_email_password_info) } catch (e: Exception) { null }

            val emailEnabled = isPremium && prefs.getBoolean("email_enabled", false)
            swEmail.isChecked = emailEnabled
            swEmail.isEnabled = isPremium
            swEmail.alpha = if (isPremium) 1f else 0.4f
            swEmail.thumbTintList = android.content.res.ColorStateList.valueOf(
                if (emailEnabled) 0xFF00CC44.toInt() else 0xFF888888.toInt())
            swEmail.trackTintList = android.content.res.ColorStateList.valueOf(
                if (emailEnabled) 0xFF003311.toInt() else 0xFF222222.toInt())

            if (isPremium) {
                etFrom.setText(prefs.getString("email_from", ""))
                etPass.setText(prefs.getString("email_password", ""))
                etTo.setText(prefs.getString("email_to", ""))
                etFrom.isFocusableInTouchMode = true
                etPass.isFocusableInTouchMode = true
                etTo.isFocusableInTouchMode = true
            } else {
                for (et in listOf(etFrom, etPass, etTo)) {
                    et.isFocusable = false; et.alpha = 0.4f
                }
                btnTest.isEnabled = false; btnTest.alpha = 0.4f
                btnSave.isEnabled = false; btnSave.alpha = 0.4f
            }

            swEmail.setOnCheckedChangeListener { _, checked ->
                if (!isPremium) { swEmail.isChecked = false; showUpgradeDialog(getString(R.string.email_alerts_label)); return@setOnCheckedChangeListener }
                prefs.edit().putBoolean("email_enabled", checked).apply()
                swEmail.thumbTintList = android.content.res.ColorStateList.valueOf(
            ...
            btnInfo?.setOnClickListener {
                androidx.appcompat.app.AlertDialog.Builder(this)
                    .setTitle(getString(R.string.email_app_password_info))
                    .setMessage(getString(R.string.email_app_password_body))
                    .setPositiveButton("OK", null).show()
            }

            btnSave?.setOnClickListener {
                if (!isPremium) { showUpgradeDialog(getString(R.string.email_alerts_label)); return@setOnClickListener }
                prefs.edit()
                    .putString("email_from", etFrom.text.toString().trim())
                    .putString("email_password", etPass.text.toString().trim())
                    .putString("email_to", etTo.text.toString().trim())
                    .apply()
                android.widget.Toast.makeText(this, getString(R.string.email_saved), android.widget.Toast.LENGTH_SHORT).show()
            }
            ...
            btnTest?.setOnClickListener {
                ...
                EmailAlertManager.sendTest(this) { success, msg ->
                    runOnUiThread {
                        btnTest.isEnabled = true
                        btnTest.text = getString(R.string.email_test_btn)
                        android.widget.Toast.makeText(this,
                            if (success) getString(R.string.email_test_success)
                            else getString(R.string.email_test_fail) + "\n" + msg,
                            android.widget.Toast.LENGTH_LONG).show()
                    }
                }
            }
        } catch (e: Exception) { Log.e("TT", "Email section: ${e.message}") }
